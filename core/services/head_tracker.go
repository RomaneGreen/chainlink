...
func (ht *HeadTracker) handleNewHighestHead(head models.Head) error {
	fmt.Println("HANDLE NEW HIGHEST HEAD:", head.Number, head.Hash)
...
// // backfill fetches all missing heads up until the base height
// func (ht *HeadTracker) backfill(ctx context.Context, head models.Head, baseHeight int64) error {
// 	fmt.Println("BACKFILL", head.Number)
// 	if head.Number <= baseHeight {
// 		return nil
// 	}
// 	mark := time.Now()
...
// 	for i := head.Number - 1; i >= baseHeight; i-- {
// 		// NOTE: Sequential requests here mean it's a potential performance bottleneck, be aware!
// 		fmt.Println("  - backfill try fetch head:", head.ParentHash.Hex())
// 		existingHead, err := ht.store.HeadByHash(head.ParentHash)
// 		if err != nil {
// 			return errors.Wrap(err, "HeadByHash failed")
// 		}
// 		if existingHead != nil {
// 			fmt.Println("  - already have", existingHead.Number, existingHead.Hash)
// 			head = *existingHead
// 			continue
// 		}
...
// 	if err != nil {
// 		ht.handleBackfillError(err, nil)
// 		return err
// 	}

// 	for i, maybeHead := range maybeHeads {
// 		if maybeHead.Error != nil {
// 			ht.handleBackfillError(maybeHead.Error, headNumbers[i])
// 			break
// 		}
// 		fmt.Println("INSERT HEAD", maybeHead.Header.Number, maybeHead.Header.Hash.Hex())
// 		err := ht.store.IdempotentInsertHead(maybeHead.Header)
// 		if err != nil {
// 			ht.handleBackfillError(err, headNumbers[i])
// 			break
// 		}
// 		fetched++
// 	}
...
// func (ht *HeadTracker) handleBackfillError(err error, i *big.Int) {
// 	if err == nil {
// 		return
// 	}
// 	var loggerArgs []interface{}
// 	if i != nil {
// 		loggerArgs = append(loggerArgs, "headNum", i.Int64())
// 	}
// 	if errors.Cause(err) == ethereum.NotFound {
...
func (ht *HeadTracker) subscribeToHead() error {
	...
	ht.headers = make(chan *models.Head, headsBufferSize)
